...
<el-form-item label="客户全称">
  ...
  <el-form-item label="审核意见">
  	 <el-select v-model="form.shzt" placeholder="请选择审核意见">
	...
	export default{
    	...
    	this.loading=true
				this.$http({
  				...
  				data: {id:this.form.id,shzt:this.form.shzt}
				}).then(response=>{
					this.$message({message:'审核成功',type:'success',showClose:true});
					this.loading=false
				}).catch(error => {
					this.loading=false
					if (error.response) {
     			 // 请求已发出，但服务器响应的状态码不在 2xx 范围内
      					if(error.response.status==403){
      						...
      						this.$message({message:'审核失败',type:'error',showClose:true});
      					}
   					 } else {
      					this.$message({message:'服务器异常，请联系管理员',type:'error',showClose:true});
    				}
